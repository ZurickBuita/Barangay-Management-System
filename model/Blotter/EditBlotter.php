...
if (isset($_SESSION['username'])) {
	...
	if (!empty($id) && !isset($response['status'])) {
		$query = "UPDATE `blotter` SET `complainant`='$complainant',`respondent`='$respondent',`victim`='$victim',`type`='$type',`location`='$location',`date`='$date',`time`='$time',`details`='$details',`status`='$status' WHERE `id`='$id'";
		$result = $conn->query($query);

		if ($result) {
			$response['message'] = 'Blotter has been updated!';
		...
		$response['message'] = 'No Blotter ID found!';
		$response['status'] = 'error';
	}
}
