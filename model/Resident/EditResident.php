    ...
    if(!isset($response['status'])) { 
		...
		if (!empty($id)) {
			if (!empty($img)) {
				...
				$query = "UPDATE `residents` SET `img`= '$resident_img', `national_id`='$national_id',`citizenship`='$citizenship',`firstname`='$firstname',`middlename`='$middlename',`lastname`='$lastname',`birthdate`='$birthday',`age`='$age',`civilstatus`='$civilstatus',`sex`='$sex',`is_voter`='$is_voter',`email`='$email',`occupation`='$occupation',`address`='$address',`is_deceased`='$is_deceased',`purok_id`='$purok_id',`is_indigenous`='$is_indigenous',`salary`='$salary',`updated_at`=NOW() WHERE id=$id;";
				// store the img
				move_uploaded_file($_FILES['img']['tmp_name'], $target);
				// Delete the previous img
				$query1 = "SELECT img FROM `residents` WHERE id=$id LIMIT 1";
				$result1 = $conn->query($query1);
				$row = $result1->fetch_assoc();
				$prev_img = $row['img'];
				if(file_exists($prev_img)) {
					unlink("../../storage/resident_img/". $prev_img);
				}
			} else {
				$query = "UPDATE `residents` SET `national_id`='$national_id',`citizenship`='$citizenship',`firstname`='$firstname',`middlename`='$middlename',`lastname`='$lastname',`birthdate`='$birthday',`age`='$age',`civilstatus`='$civilstatus',`sex`='$sex',`is_voter`='$is_voter',`email`='$email',`occupation`='$occupation',`address`='$address',`is_deceased`='$is_deceased',`purok_id`='$purok_id',`is_indigenous`='$is_indigenous',`salary`='$salary',`updated_at`=NOW() WHERE id=$id;";
			}
			
			$result = $conn->query($query);

			if ($result) {
				$response['message'] = 'Resident has been updated!';
			...
			$response['message'] = 'No Resident ID found!';
			$response['status'] = 'error';
		}
    }
}
