<?php
    ...
    if(!empty($id)) {
        ...
        $query['select'] = "SELECT img FROM `residents` WHERE id=$id LIMIT 1";
        $result['select'] = $conn->query($query['select']);
        $row = $result['select']->fetch_assoc();
        $img = $row['img'];

        $query['delete'] = "DELETE FROM residents WHERE id = '$id'";
        $result['delete'] = $conn->query($query['delete']);

        if ($result['delete'] == true) {
            $img_path = "../../storage/resident_img/".$img;
            if(!empty($img)) {
            ...
            $response['message'] = 'Resident has been removed!';
        } else {
